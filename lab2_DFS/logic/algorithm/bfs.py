def bfs(graph, start, goal, order="asc"):
    queue = [[start]]
    visited = set()  # Start with empty visited set
    visited_edges = []
    ...
    while queue:
        ...
        # Add to visited only when we process the node
        visited.add(current)
        ...
        if current == goal:
            ...
            yield path, path_edges, visited_edges, queue_nodes, visited
            return
        ...
        queue_nodes = [p[-1] for p in queue]
        yield path, current_path_edges, visited_edges, queue_nodes, visited

        # Process neighbors
        for neighbor in neighbors:
            if neighbor not in visited and neighbor not in queue_nodes:  # Check both visited and queue
                if neighbor == goal:
                    new_path = path + [neighbor]
                    path_edges = [(new_path[i], new_path[i + 1]) for i in range(len(new_path) - 1)]
                    visited_edges.append((current, neighbor))
                    visited.add(neighbor)  # Add goal to visited
                    stats = {
                    ...
                    print(f"!!! Search efficiency: {stats['path_length']}/{len(visited)} = {stats['efficiency']:.1f}%")
                    queue_nodes = [p[-1] for p in queue]
                    yield new_path, path_edges, visited_edges, queue_nodes, visited
                    return
    ...
    print(f"!!! Total nodes explored: {len(visited)}")
    queue_nodes = [p[-1] for p in queue]
    yield [], [], visited_edges, queue_nodes, visited
